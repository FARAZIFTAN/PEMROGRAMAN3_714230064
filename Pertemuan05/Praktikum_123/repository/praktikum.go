...
func GetMahasiswaByNPM(ctx context.Context, npm int) (mhs model.Mahasiswa, err error) {
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			return mhs, fmt.Errorf("mahasiswa dengan NPM %v tidak ditemukan", npm) // NPM tidak ditemukan
		}
		fmt.Printf("GetMahasiswaByNPM (FindOne): %v\n", err)
		return mhs, err // Error lain saat mencari mahasiswa
	}
	return mhs, nil
